...
def hard_negative_mining(item):
    ...
    try:
        ...
        top_20_retrieval = dense_retrieval_subqueries_for_finetune(sub_questions, all_subquestion_list, index_file, corpus_index, corpus, top_k=40)
        top_k_retrieval = random_sample(top_20_retrieval, args.top_k)
...
def train(args, logger: logging.Logger):
    ...
    train_loss = losses.TripletLoss(model, distance_metric=losses.TripletDistanceMetric.EUCLIDEAN, triplet_margin=args.margin)
